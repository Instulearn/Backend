Feature: As an administrator, I should be able to access the detailed information of the product faq with the
  ...
  Scenario Outline: It should be verified that when a GET request is sent to the /api/productfaq/{id} endpoint with
  valid authorization information and an unregistered (id), the status code returned is 203, the remark in the response
  body is “failed” and the message is “There is not product faq for this id.”

    * The api user constructs the base url with the "admin" token.
    # Api kullanicisi "admin" token ile base urli olusturur
    * The api user sets "api/productfaq/<id>" path parameters.
    # Api kullanicisi "api/productfaq/<id>" path parametrelerini olusturur
    * The api user sends a GET request and saves the returned response.
    # Api kullanicisi GET request gonderir ve donen responsei kaydeder
  ...
  Scenario Outline: when a GET request is sent without (id), the status code returned is 203, the remark in the
  ...
  Scenario Outline: When a GET request is sent to the /api/productfaq/{id} endpoint with invalid token authorization
  information and correct data (id), it should be verified that the status code returned is 401 and the message
