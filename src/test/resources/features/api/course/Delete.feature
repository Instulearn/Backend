...
Background:
  ...
  * Background executed
  ...
  matches the {id} path parameter and Verify that the deleted course via API is successfully deleted by sending a GET request to
  /api/course/{id} using the Deleted Course ID from the response.

    * The api user sends a DELETE request with "admin" token and saves the returned response.
    * The api user verifies that the status code is 200
    * The api user verifies that the "remark" information in the response body is "success"
    * The api user verifies that the "Message" information in the response body is "Successfully Deleted."
    * The api user verifies that the "Deleted Course ID" information in the response body is the same as the id path parameter in the endpoint
    * The api user sends a GET request with the Deleted Course ID information and saves the returned response.
  ...
  Scenario: When a DELETE request is sent to the /api/deleteCourse endpoint with valid authorization information
  and without (id), it should be verified that the returned status code is 203, the remark information in the response body is
