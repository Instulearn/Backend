Feature: As an administrator, I want to be able to delete course price plan information with the specified id number via API connection.
  ...
  matches the {id} path parameter.
    ...
    * The API user sets "api/deletePricePlan" path parameters  sends a DELETE request for id 210 to coursplanprice and records the returned response.
    # Api kullanicisi "api/deleteCategory/id" path parametrelerini olusturur, DELETE request gonderir ve donen responsei kaydeder
    # ***Bu stepte yazdığınız id, son otomatik kullanılır
