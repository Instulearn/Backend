Feature: As an administrator I want to be able to access courses categories via API connection.
  ...
  # Api kullanicisi response bodydeki remark bilgisinin "success" oldugunu dogrular
    * The api user verifies the <creator_id>, <webinar_id>, <bundle_id>, <start_date>, <end_date>, <discount> ,<capacity>,<order>, <created_at>, <updated_at>, <deleted_at>,<title>, <id>, <ticket_id>, "<locale>","<title>" information of the item at <dataIndex> in the response body.
  # Api kullanıcısı response body icindeki <dataIndex> indexe sahip olanin "<slug>", "<icon>", <order>, <id>, <category_id>, "<locale>" ve "<title>" bilgilerini doğrular.
    ...
    Examples:
      | creator_id | webinar_id | bundle_id | start_date | end_date  | discount| capacity| order|created_at |updated_at|deleted_at|title|id | ticket_id|locale|title   | dataIndex
      | 1589       | 2658       | null      | 1750550400 | 1752451200| 15      | 2       | null | 1750466029| null     |null      |null |169| 196      | en   |Discount| 3
  ...
  "Unauthenticated."
    ...
    * The api user sends a GET request, saves the returned response, and verifies that the status code is '401' with the reason phrase Unauthorized.
    # Api kullanicisi GET request gonderir, donen responsei kaydeder, status codeun '401' ve reason phrase bilgisinin Unauthorized oldugunu dogrulargit a