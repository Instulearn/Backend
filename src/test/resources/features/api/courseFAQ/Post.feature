Feature: As an administrator I want to be able to create a new course faq record via API connection.

  Scenario: When a POST body with valid authorization information and correct data (title, answer, product_id) is
  sent to /api/addCoursefaq endpoint, it should be verified that the status code returned is 200, the remark
  in the response body is “success” and the Message is “Successfully Added.”.
    ...
    * The api user prepares a post request body to send to the api addCategory endpoint for Coursefaq.
    # Api kullanicisi api addCategory endpointine gondermek icin bir post request body hazirlar
    * The api user sends a POST request and saves the returned response for Coursefaq.
    ...
    * The api user verifies that the "Message" information in the response body is "Successfully Added." for Coursefaq.
    # Api kullanicisi response bodydeki Message bilgisinin "Successfully Added." oldugunu dogrular


  Scenario: When a POST request is sent to /api/addCoursefaq endpoint without valid authorization information and data,
  ...
  “The product id field is required. (and 2 more errors)”
    ...
    * The api user sets "api/addCoursefaq" path parameters.
    # Api kullanicisi "api/addCategory" path parametrelerini olusturur
    * The api user prepares a post request without any data to send to the api addCategory endpoint for Coursefaq.
    # Api kullanicisi api addCategory endpointine gondermek için data içermeyen bir post request hazirlar
    * The api user sends a POST request and saves the returned response for Coursefaq.
    # Api kullanicisi POST request gonderir ve donen responsei kaydeder
    * The api user verifies that the status code is 422 for Coursefaq.
    # Api kullanicisi status codeun 422 oldugunu dogrular
    * The api user verifies that the "message" information in the response body is "The product id field is required. (and 2 more errors)" for Coursefaq.
  ...
  (title, answer, product_id) to /api/addCoursefaq endpoint, it should be verified that the status code returned is
  401 and the message information in the response body is “Unauthenticated.”
    ...
    * The api user sends a POST request and saves the returned response for Coursefaq.
    # Api kullanicisi POST request gonderir ve donen responsei kaydeder
    * The api user verifies that the status code is 401 for Coursefaq.
    # Api kullanicisi status codeun 401 oldugunu dogrular
    * The api user verifies that the "message" information in the response body is "Unauthenticated." for Coursefaq.
    # Api kullanicisi response bodydeki message bilgisinin "Unauthenticated." oldugunu dogrular


  Scenario Outline: The creation of the new course faq record to be created through the API must be verified
  through the API. (It can be verified that the record was created by sending a GET request to the
  /api/Coursefaq/{id} endpoint with the Added Product Faq ID returned in the response body).
