...
Scenario Outline: When a GET request is sent to /api/product/{id} endpoint with valid authorization information,
...
Scenario: Geçersiz ID ve ID olmadan request gönderme senaryoları
    * The api user constructs the base url with the "admin" token.
    # Api kullanicisi "admin" token ile base urli olusturur
    * The api user sets "api/product/1" path parameters.
    # Api kullanicisi "api/product/1" path parametrelerini olusturur
